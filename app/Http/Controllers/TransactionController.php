...
class TransactionController extends Controller
{
    ...
    private $view     =  "subDealers/transactions.";
    private $action   =  "/dashboard/subDealers/transactions";
    private $db_key   =  "id";
    private $perpage = 5;
    ...
    public function create(Request $request,$id=NULL)
    {
        if($request->isMethod('POST')){
            ...
            // }
            // $result = Transaction::where("subdealer_id","=",$data["subdealer_id"])->first();

            // if($result){
            //     $result->bill_amount = $result->bill_amount + $data['bill_amount'];
            //     $result->received_amount =  $result->received_amount + $data['received_amount'];
            //     $result->save();
            //     $response = array('flag'=>true,'msg'=>$this->singular.' is added sucessfully.','action'=>'reload');
            //     echo json_encode($response); return;
            // }else {
            //     $Areas         = new Transaction;
            //     $Areas->insert($data);
            //     $response = array('flag'=>true,'msg'=>$this->singular.' is added sucessfully.','action'=>'reload');
            //     echo json_encode($response); return;
            // }
