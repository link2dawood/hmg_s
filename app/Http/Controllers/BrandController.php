...
class BrandController extends Controller
{
    ...
    private $view     =  "brands.";
    private $action   =  "/dashboard/brands";
    private $db_key   =  "id";
    private $perpage = 5;
    ...
    public function list(Request $request)
    {
        ...
        GET RECORDS
        */
        $records   = Brand::where("office_id","=",Auth::user()->office)->where("company_id","=",Auth::user()->company);
    ...
    public function create(Request $request)
    {
        if($request->isMethod('POST')){
            ...
            $is_save             = Brand::where('name','=',
                                                $data['name'])
                                                ->count();
            if($is_save > 0)    {
                $response = array('flag'=>false,'msg'=>$this->singular.' with name exist.');
